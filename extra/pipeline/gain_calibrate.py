            ...
            for j in plnums:
                ...
                for h in range(first_len):
                    ...
                    try:
                        ...
                        if len(pre_on_times) != 0 and len(pre_off_times) != 0:
                            start2 = time.perf_counter()

                            pre_on_params, pre_on_unc = self.rcr((pre_on_times, pre_on_vals))
                            
                            end2 = time.perf_counter()
                            print("RCR Time", end2 - start2)
                    ...
                    try:
                        ...
                        if len(post_on_times) != 0 and len(post_off_times) != 0:
                ...
                print(pre_cal_deltas, post_cal_deltas)
                for h in range(first_len):
                    if pre_cal_deltas[h] and post_cal_deltas[h]:
                    ...
                    elif pre_cal_deltas[h] and not post_cal_deltas[h]:
                        pre_avg = average_neighbors(pre_cal_deltas, h)
                        pre_unc_avg = average_neighbors(pre_cal_uncs, h)
                        ...
                        print(f"IFNUM={i}, PLNUM={j}, CH={h} → pre={pre_avg}±{pre_unc_avg}")

                    elif post_cal_deltas[h] and not pre_cal_deltas[h]:
                        post_avg = average_neighbors(post_cal_deltas, h)
                        post_unc_avg = average_neighbors(post_cal_uncs, h)

                        weights = np.array([1 / post_unc_avg, 1 / post_unc_avg])
                        delta = np.average([post_avg, post_avg], weights=weights)
                        self.data['DATA'][:, h] /= delta

                        print(f"IFNUM={i}, PLNUM={j}, CH={h} → post={post_avg}±{post_unc_avg}")
                ...
                # Left plot: average spectrum
                spectrum = np.mean(filtered_data['DATA'], axis=0)
                ch_indices = np.arange(len(spectrum))


                # Right plot: deltas
                pre_deltas = np.array(pre_cal_deltas, dtype=float)
                post_deltas = np.array(post_cal_deltas, dtype=float)
                # Handle None values: replace with nan for plotting
                pre_deltas = np.array([np.nan if v is None else v for v in pre_deltas])
                post_deltas = np.array([np.nan if v is None else v for v in post_deltas])
                # delta_diff = np.abs(pre_deltas - post_deltas)

                self._pre_cal_deltas = pre_deltas
                self._post_cal_deltas = post_deltas
                ...
                # ax1.set_title("Mean Spectrum (Filtered IFNUM=0, PLNUM=0)")
                # ax1.grid(True)

                # # --- Right: Calibration deltas ---
                ...
                # fig.tight_layout()
                # plt.savefig("Calibrated Spectrum Data Plot", dpi=300)
                # self.data = self.old_data



    def animate_gain_deltas(self, window_range=(100, 1000, 20), output_file="gain_deltas_animation_low_all.mp4"):
        """
        Animate the effect of different smoothing window sizes on the gain deltas.
        Each frame will show the plots for a different window size.
        """

        # Prepare the figure and axes only once
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

        # Precompute the list of windows to iterate over
        window_sizes = list(range(*window_range))

        # This function will be called for each frame
        def update(frame_idx):
            window = window_sizes[frame_idx]
            print(f"[INFO] Animating frame {frame_idx+1}/{len(window_sizes)}, window={window}")
            ...
            try:
                # --- Run your compute_gain_deltas logic with a given window ---
                # Here, we assume you have refactored your function to accept a window parameter
                # For example, you could factor the inner part into a helper:
                self.compute_gain_deltas(window=window)

                # After compute_gain_deltas runs, we can read self._pre_cal_deltas etc.
                mask = (self.data['IFNUM'] == 0) & (self.data['PLNUM'] == 0)
                filtered_data = self.data[mask]
                ...
                pre_deltas = np.array([np.nan if v is None else v for v in self._pre_cal_deltas])
                post_deltas = np.array([np.nan if v is None else v for v in self._post_cal_deltas])
                delta_diff = np.abs(pre_deltas - post_deltas)
                ...
                if post_deltas[0]:
                    ax2.plot(ch_indices, post_deltas, label="Post-Cal", color='orange', alpha=0.6, linewidth=0.6)
                if pre_deltas[0] and post_deltas[0]:
                    ax2.plot(ch_indices, delta_diff, label="|Pre-Post|", color='red', alpha=0.8, linewidth=0.6)
                ax2.set_xlabel("Channel Index")
                ax2.set_ylabel("Delta")
                ax2.set_title("Calibration Deltas")
                ...
                print(f"[ERROR] Frame {frame_idx} (window={window}) failed: {type(e).__name__}: {e}")

                # Optionally, plot something placeholder so animation has a frame
        ...
        ani = animation.FuncAnimation(fig, update, frames=len(window_sizes), repeat=False)
...
if __name__ == "__main__":
    filepath = "C:/Users/starb/Downloads/0137029(1).fits"
